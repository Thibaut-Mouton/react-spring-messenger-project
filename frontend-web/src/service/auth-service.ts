...
export default class AuthService {
    ...
    public fetchMessages (id: number) {
        return instance.post(API_URL + 'fetchMessages', { id });
    }

    public addUserToGroup (userId: number | string, groupUrl: string) {
        return instance.get(API_URL + 'user/add/' + userId + '/' + groupUrl);
    }

    public fetchAllUsersInConversation (groupUrl: string) {
        return instance.get(API_URL + 'users/group/' + groupUrl, {});
    }

    public fetchAllUsersWithoutAlreadyInGroup (groupUrl: string) {
    ...
    public leaveConversation (userIdToRemove: number, groupId: string) {
        return instance.get(API_URL + 'user/leave/' + userIdToRemove + '/group/' + groupId);
    }

    public removeUserFromConversation (userIdToRemove: string | number, groupUrl: string) {
        return instance.get(API_URL + 'user/remove/' + userIdToRemove + '/group/' + groupUrl);
    }

    public removeAdminUserInConversation (userIdToRemove: string | number, groupUrl: string) {
        return instance.get(API_URL + 'user/remove/admin/' + userIdToRemove + '/group/' + groupUrl);
    }

    public grantUserAdminInConversation (userIdToGrant: number | string, groupId: string) {
