...
import { Button, FormControlLabel, Switch, Toolbar, Typography } from '@mui/material';
...
import { setGroupName } from '../reducers';
...
export const HeaderComponent: React.FunctionComponent<HeaderComponentType> = () => {
    ...
    useEffect(() => {
        new AuthService().testRoute().then((res: AxiosResponse<UserModel>) => {
            ...
            const group = groups.find((group) => group.url === firstGroupUrl);
            if (group) {
                dispatch(setGroupName({ groupName: group.name }));
            }
            setUser(res.data);
        }).catch((err) => {
            console.log(err);
